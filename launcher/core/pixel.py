...
def fit(pm: QPixmap, w: int, h: int, focus: float = 0.5) -> QPixmap:
    """Scale a sprite to fill w*h, cropping the overflow, pixels intact.

    `focus` biases the vertical crop: 0 keeps the top, 1 keeps the bottom.
    """
    ...
    return big.copy(QRect((tw - w) // 2, round((th - h) * focus), w, h))
